...
RECORD_COMPLAINT = {
    ...
    "parameters": {
        ...
        "properties": {
            "category": {
                "type": "STRING",
                "description": "ประเภท/หมวดหมู่ของปัญหา — จัดเองจากสิ่งที่ผู้ใช้เล่า (ผู้ใช้ไม่ต้องพูดชื่อหมวดตรงๆ "
                "การจัดหมวดไม่ถือว่าเดา) เลือกได้เฉพาะค่าในลิสต์นี้: "
                + ", ".join(CATEGORIES),
            },
...
- ห้ามเดา/เติม/ขยาย "ข้อเท็จจริง" ที่ผู้ใช้ไม่ได้บอกเองเด็ดขาด โดยเฉพาะสถานที่ เช่น ถ้าผู้ใช้พิมพ์ชื่อสถานที่ยังไม่จบ (เช่น "ซอย 7 ย่านหนอ") ห้ามเดาต่อเป็นชื่อเต็มเอง (เช่น "หนองจอก") ให้เก็บตามตัวอักษรที่พิมพ์มาจริง หรือถามย้ำก่อน
- แต่การ "ตีความ" สิ่งที่ผู้ใช้เล่าเพื่อลงช่องข้อมูล ทำได้ตามปกติ ไม่ถือว่าเดา: จัดหมวดหมู่ (category) ให้ตรงที่สุดจากลิสต์, ประเมินความรุนแรง (severity) จากผลกระทบที่เขาเล่า, สรุปหัวข้อ (title) สั้นๆ จากเรื่องของเขา — ห้ามเว้นว่างหรือเลี่ยงประเมินเพราะกลัวผิดกฎข้อบน สิ่งที่ห้ามคือ "กุข้อเท็จจริงใหม่" ที่ผู้ใช้ไม่ได้บอกเท่านั้น
...
- ห้ามเดา/เติม/ขยาย "ข้อเท็จจริง" ที่ผู้ใช้ไม่ได้บอกเอง โดยเฉพาะสถานที่ — ถ้าพิมพ์ชื่อสถานที่มาไม่ครบ ให้เก็บตามตัวอักษรที่พิมพ์มาจริง หรือถามย้ำก่อน ห้ามเดาต่อเป็นชื่อเต็มเอง
- แต่การจัดหมวดหมู่ (category) ประเมินความรุนแรง (severity) จากผลกระทบ และสรุปหัวข้อ (title) จากเรื่องที่เขาเล่า ทำได้ตามปกติ ไม่ถือว่าเดา — ห้ามเว้นว่างเพราะกลัวผิดกฎข้อบน
